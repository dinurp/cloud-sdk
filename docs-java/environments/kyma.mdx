...
2. In the Kyma dashboard, select your Kyma namespace (top right hand side of the screen), navigate to `Service Management > Service Instances`, and click on the `+ Create Service Instance` button.

![Service Instance Creation](sap-btp-kyma-environment/btp-kyma-dashboard-destination-service-new.png)

3. Configure your new service instance by entering a name, Offering Name as `destination` and a desired plan.
...
5. Click on the `+ Create BTP Service Binding` button.

![Service Binding Creation](sap-btp-kyma-environment/btp-kyma-dashboard-destination-binding-new.png)

6. Configure your new service binding by entering a name, selecting the previously created service instance and a name for secret.

7. Finalize the service binding creation by confirming your configuration with a click on the `Create` button.

![Destination Binding Creation](sap-btp-kyma-environment/btp-kyma-dashboard-destination-binding-creation.png)
...
spec:
  ...
  servicePlanName: <your-desired-plan>
---
apiVersion: services.cloud.sap.com/v1
kind: ServiceBinding
metadata:
...
1. Open your Kyma dashboard, select your namespace (see previous section), navigate to `Service Management > Service Instances`, and click on the `+ Create Service Instance` button.

![XSUAA Service Selection](sap-btp-kyma-environment/btp-kyma-dashboard-xsuaa-service-new.png)

2. Configure your new service instance by entering a name, Offering Name as `xsuaa` and a desired plan.

3. Switch to Advanced mode and enter configure an Instance Parameter xsappname with value desired app name in role configuration.
...
![Service Binding Creation](sap-btp-kyma-environment/btp-kyma-dashboard-xsuaa-binding-new.png)

6. Configure your new service binding by entering a name, selecting the previously created service instance and a name for secret.

7. Finalize the service binding creation by confirming your configuration with a click on the `Create` button.

![XSUAA Binding Creation](sap-btp-kyma-environment/btp-kyma-dashboard-xsuaa-binding-creation.png)
...
kind: ServiceBinding
metadata:
  name: my-app-xsuaa-service-binding
spec:
...
5. Update the already created XSUAA service instance with few parameters like `oauth2-configuration`.
